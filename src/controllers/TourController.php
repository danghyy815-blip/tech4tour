    ...
    public function formUpdateTour()
{
    ...
    // Chuẩn bị selected policies
    $selectedPolicies = [];
    if (!empty($_POST['chinh_sach_id'])) {
        $selectedPolicies = $_POST['chinh_sach_id']; // dữ liệu submit lỗi
    } elseif (!empty($tour['chinh_sach_id'])) {
        $selectedPolicies = array_map('intval', explode(',', $tour['chinh_sach_id'])); // dữ liệu DB
    }

    // Chuẩn bị dữ liệu cũ (old) khi submit lỗi
    $old = [];
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        $old = [
    ...
    public function updateTour()
{
    ...
    public function detailTour()
    {
        ...
        // Lấy thông tin chi tiết các chính sách
        $policyModel = new ChinhSach();
